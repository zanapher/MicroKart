...
class Track(object):
	...
	def __init__(self, window, race, track_num):
		...
		self.map = pyglet.sprite.Sprite(pyglet.resource.image("%s.png" % track_num), batch=window.batch, group=self.bg_group)
		...
		try:
			...
			print "Beacons map not found, creating one."
	...
	def make_beacons(self):
		...
		print "%s beacons created." % len(beacons)
		return beacons
